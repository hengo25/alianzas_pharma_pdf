...
def productos():
    ...
    por_pagina = int(request.args.get("por_pagina", 9))  # 9 productos (3x3)

    lista = obtener_productos()
    lista.sort(key=lambda p: (p.get("nombre") or "").lower())  # orden alfabético
...
def productos_agregar():
    ...
    if not (nombre and precio and imagen):
        flash("⚠️ Faltan datos para agregar el producto.", "danger")
...
def productos_pdf():
    ...
    if not ids:
        flash("⚠️ No seleccionaste productos para el PDF.", "warning")
        return redirect(url_for("productos"))
    ...
    doc.build(story)
    buffer.seek(0)
    return send_file(buffer, as_attachment=True, download_name="catalogo_alianzas_pharma.pdf", mimetype="application/pdf")
